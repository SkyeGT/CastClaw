...
![TUI + CLI](https://img.shields.io/badge/TUI%20%2B%20CLI-Ready-green) ![Python ML 后端](https://img.shields.io/badge/Python%203.10+-Compatible-blue) ![License](https://img.shields.io/badge/License-MIT-yellow)
...
启动 TUI 后，使用 `Ctrl+1/2/3` 切换智能体，然后开始：
...
**LLM 提供商：** 通过 Vercel AI SDK 支持 20+ 个提供商（Anthropic、OpenAI、Google、OpenRouter 等）。

```bash
export ANTHROPIC_API_KEY=sk-ant-...
# 或使用 OPENAI_API_KEY、GOOGLE_GENERATIVE_AI_API_KEY 等
```
...
阶段二（Planner）    → 定性 & 定量预测前分析
        ↓
阶段三（Planner）    → 模型技能生成与人工审核
        ↓
阶段四（Forecaster） → 并行实验循环 + 人机协作（HITL）反馈
        ↓
阶段五（Critic）     → 最终报告、可视化、多模型对比
```
...
- **[完整中文使用指南](./docs/zh/usage.md)** — 使用流程、工作流详解、技能参考
- **[English Guide](./docs/en/usage.md)** — 完整英文文档
